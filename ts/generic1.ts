...
export function mergeArraysByKeys<T1 extends Record<string, any>, T2 extends Record<string, any>>(
  array1: T1[],
  array2: T2[],
  key1: keyof T1,
  key2: keyof T2
): (T1 & T2)[] {
  const map = new Map<string | number, T1 & T2>();

  array1.forEach((item) => {
    const key = item[key1];
    if (typeof key === 'string' || typeof key === 'number') {
      map.set(key, { ...item } as T1 & T2);
    }
  });
